...
def extract_text_blocks(pdf_path):
    """
    Extract and print all text blocks from the OCR PDF for inspection.
    """
    doc = fitz.open(pdf_path)
    print("\nExtracted text blocks from the OCR PDF:")
    ...
    for page_number in range(len(doc)):
        ...
        for block in blocks:
            ...
            print(text.strip())  # Print the text block content
...
scanned_pdf_path = "K3-1.pdf"  # Path to the original scanned PDF
ocr_pdf_path = "ocr.pdf"  # Specify a valid path for the OCR PDF
...
try:
    ...
    # Step 2: Extract and print all text blocks from the OCR PDF for inspection
    extract_text_blocks(ocr_pdf_path)
